...
public class FarmPersonalCommand extends AbsPersonalCommand {
	...
	public Response execute() {
		...
		if (user.getItems().isEmpty()) {
			...
			List<Item> todayFarmUserItems = itemsService.getFarmableItemsByDay(user.getItems(), dayOfWeek);
			...
			if (todayFarmUserItems.isEmpty()) {
				...
				List<Item> tomorrowFarmUserItems = itemsService.getFarmableItemsByDay(user.getItems(), nextDayOfWeek);
				...
				if (tomorrowFarmUserItems.isEmpty()) {
					...
					answer.append(String.join(", ",
							tomorrowFarmUserItems.stream().map(i -> i.getName()).collect(Collectors.toList())));
					answer.append(".");
				}
				...
				if (dayOfWeek != Calendar.SUNDAY) {
					try {
						...
						answer.append(String.join(", ",
								todayFarmUserItems.stream().map(i -> i.getName()).collect(Collectors.toList())));
						answer.append(".");
